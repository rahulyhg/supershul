...
if( !class_exists( 'HKB_View_Count' ) ){
    ...
    class HKB_View_Count {
        ...
        function __construct() {
            ...
            //register_deactivation_hook( __FILE__, array( 'HKB_View_Count', 'hkb_visits_plugin_deactivation_hook' ) );
        }

        /**
        * Increment article view count on wp_head
        */
        function ht_kb_increase_article_viewcount(){;
            ...
            if('ht_kb_article'==$queried_object_data['object_type']){
                $this->ht_kb_set_post_views($queried_object_data['object_id']);
            }           
        ...
        */
        function ht_kb_add_post_view_data_to_table($data){
            ...
            //visit_id,  object_type, object_id, user_id, user_ip, datetime, duration
            $query = "INSERT INTO {$wpdb->prefix}" . HT_VISITS_TABLE .   " ( visit_id ,  object_type, object_id, user_id, user_ip, datetime, duration)
            VALUES ($visit_id, '$object_type', $object_id, $user_id, '$user_ip', '$mysqltime', $duration)";
        ...
        */
        function ht_kb_get_post_views($post_id){
            global $wpdb;
        ...
        */
        function ht_kb_set_post_views($post_id) {
            $visits_total = $this->ht_kb_get_post_views($post_id);
        ...
        */
        function upgrade_article($post_id){
            //upgrade required
            if($this->ht_kb_get_post_views($post_id)!=0){
                //do nothing
            } else {
                //convert
                ...
                $this->ht_kb_set_post_views($post_id);
            }
...
//run the module
if( class_exists( 'HKB_View_Count' ) ){
    $hkb_view_count_init = new HKB_View_Count();
}