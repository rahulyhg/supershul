    ...
    class HT_Knowledge_Base_Custom_Article_Order {
        ...
        */
        function add_ht_kb_article_ordering_menu(){
            ...
            add_submenu_page( 'edit.php?post_type=ht_kb', $page_title, $menu_title, 'manage_options', 'ht_kb_article_ordering_page', array($this, 'display_ht_kb_article_ordering_page') ); 
        }
        ...
        */
        function ht_kb_sort_by_custom_check(){
            ...
            $user_sort_by = '';
            if( isset($ht_knowledge_base_options) && is_array($ht_knowledge_base_options) && array_key_exists('sort-by', $ht_knowledge_base_options) && array_key_exists('sort-order', $ht_knowledge_base_options ) ){
            ...
            if(is_admin() && is_object($screen) && ('ht_kb_page_ht_kb_article_ordering_page' == $screen->base) && 'custom' != $user_sort_by ){  
                ?>
                    <div class="error">
                        <p><?php printf( __( 'You must set the Knowledge Base <strong>Sort By</strong> to <strong>Custom</strong> to enable article ordering. 
                                        You can do this now from the <a href="%s">Knowledge Base General Settings page</a>.', 'ht-knowledge-base' ), 
                                        admin_url('edit.php?post_type=ht_kb&page=_ht_kb_options&tab=0' ) ); ?></p>
                    </div>
                <?php 
            }
        ...
        */
        function enqueue_ht_kb_article_ordering_scripts_and_styles(){
            ...
            if(  $screen->base == 'ht_kb_page_ht_kb_article_ordering_page' ) {
                ...
                wp_enqueue_script( 'ht-kb-article-ordering-script', plugins_url( 'js/hkb-admin-article-ordering-js.js', dirname(__FILE__) ), array( 'jquery' , 'jquery-effects-core', 'jquery-ui-draggable', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-ui-sortable' ), 1.0, true );              
...
//run the module
if(class_exists('HT_Knowledge_Base_Custom_Article_Order')){
    $ht_knowledge_base_custom_tax_order = new HT_Knowledge_Base_Custom_Article_Order();
}