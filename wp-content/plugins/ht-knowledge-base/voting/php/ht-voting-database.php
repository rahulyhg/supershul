    ...
    class HT_Voting_Database {
        ...
        */
        function ht_voting_get_vote_by_key($post_id, $key){
            global $wpdb;
            $get_vote_query = "SELECT *, COUNT(*) as count 
                                    FROM {$wpdb->prefix}" . HT_VOTING_TABLE  .
                                    " WHERE post_id = '{$post_id}'
                                    AND vote_key = '{$key}'
                                ";
        ...
        */
        function delete_vote_from_database($post_id, $vote_id){
            global $wpdb;
             $delete_vote_query = "DELETE
                                    FROM {$wpdb->prefix}" . HT_VOTING_TABLE  .
                                    " WHERE post_id = '{$post_id}'
                                    AND vote_id = '{$vote_id}'
                                ";

            $wpdb->query($delete_vote_query);
        ...
        */
        function save_vote_for_article($post_id, $vote=null){
            ...
            if($vote_id=='NULL'){
                //new/insert
                $query = "INSERT INTO {$wpdb->prefix}" . HT_VOTING_TABLE .   " ( vote_id ,  vote_key, post_id, user_id, user_ip, datetime, magnitude, feedback )
                VALUES ($vote_id, '$vote_key', $post_id, $user_id, '$user_ip', '$mysqltime', $magnitude, '$feedback')";
            } else {
                //update
                $query = "UPDATE {$wpdb->prefix}" . HT_VOTING_TABLE .   "
                SET  vote_key = '$vote_key', post_id = $post_id, user_id = $user_id, user_ip = '$user_ip', datetime = '$mysqltime', magnitude = $magnitude, feedback = '$feedback'
