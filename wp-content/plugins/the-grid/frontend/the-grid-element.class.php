	...
	public function add_action($args, $content = '', $class = '') {
		...
		if ($action_args) {
			...
			if ($type == 'link') {
				...
				$target = $this->base->getVar($action_args, 'link_target', 'self');
	...
	public function get_social_share_link($args = '', $class ='') {
		...
		switch ($type) {
			...
			case 'pinterest':
				...
				$href = 'https://pinterest.com/pin/create/button/?url='.$link.'&description='.$title.'&media='.$image;
				break;
			case 'linkedin':
		...
		return (isset($href) && !empty($href)) ? '<a class="tg-social-share'.$disable_class.' tg-'.$type.esc_attr($class).'" href="'.($disable_class ? null : $href).'">'.$icon.'</a>' : null;
			...
			foreach ($terms as $term) {
				...
				if (($tg_skins_preview || empty($taxonomy) || in_array($term['taxonomy'], $taxonomy)) && ($term['taxonomy'] != 'post_format' || in_array('post_format', $taxonomy))) {
					...
					if ($terms_link && !empty($term['url'])) {
						$cat .= '<a class="'. esc_attr($term['taxonomy']) .'" href="' . esc_url($term['url']) . '" rel="category">';
