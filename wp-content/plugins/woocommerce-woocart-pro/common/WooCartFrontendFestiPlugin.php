...
class WooCartFrontendFestiPlugin extends WooCartFestiPlugin
{
    ...
    public function onAddCartProductsToHead()
    {
        ...
        $items = array();
        foreach ($woocommerce->cart->cart_contents as $cartProduct) {
    ...
    public function doAjaxUpdateTotalPrice($validAjaxData, $woocommerce)
    {
        ...
        if ($passedValidation) {
            $woocommerce->cart->set_quantity($itemKey, $quantity, false);
        }
        $woocommerce->cart->calculate_totals();
        $result = array(
            "totalPrice" => $woocommerce->cart->get_cart_subtotal(),
