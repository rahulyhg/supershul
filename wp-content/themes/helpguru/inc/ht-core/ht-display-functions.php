				...
				if( !empty($terms) ){
					foreach($terms as $term) {
						...
						if ($term->parent != 0) { 
							...
							echo '<a href="' . esc_attr(get_term_link($term, 'category')) . '" title="' . sprintf( __( "View all posts in %s", 'ht-theme' ), $term->name ) . '" ' . '>' . $term->name.'</a> ';
							echo $sep;
...
if(!function_exists('ht_get_term_parents')){
	...
	function ht_get_term_parents( $id, $taxonomy, $link = false, $separator = '/', $nicename = false, $visited = array() ) {
	  ...
	  if ( $link )
	    $chain .= '<a href="' . esc_url( get_term_link( intval( $parent->term_id ), $taxonomy ) ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", 'ht-theme' ), $parent->name ) ) . '">'.$name.'</a>' . $separator;
	  else
